...
% ---
\definecolor{mdc1}{HTML}{D97757}
Krogh-Vedelsby 集成误差分解，也称为 \verb|Ambiguity Decomposition|，用于解释集成学习中一个重要现象：集成模型的性能提升不仅来自\verb|单个模型本身的准确性|，也来自不同模型之间的\verb|互补分歧|。
...
其中，\textbf{单模型平均误差越低越好，而模型之间的有效分歧越大，集成模型的误差就可能越低}。
...
表示\verb|模型之间的 ambiguity|，也就是模型分歧、多样性或互补性。
...
\begin{center}
\textbf{\textcolor{mdc1}{基模型之间需要具有有效的互补分歧。}}
\end{center}
...
\section{对多专家 AIGC 检测的启发}

在多专家 AIGC 检测中，不同检测器可能关注不同证据：

\begin{itemize}
  \item low-level artifact
  \item frequency artifact
  \item compression trace
  \item VAE reconstruction residual
  \item semantic inconsistency
  \item generator-specific cue
\end{itemize}
...
\begin{itemize}
  \item 哪些专家在某类生成器上更可靠；
  \item 哪些专家对压缩、resize、重建更鲁棒；
  \item low-level detector 和 high-level detector 分歧时应该如何判断；
  \item 多个专家一致时如何决策；
  \item 多个专家严重分歧时如何决策。
\end{itemize}
...
\section{Logit Fusion 与 Embedding Fusion}
