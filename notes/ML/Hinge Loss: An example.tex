...
\begin{itemize}
  \item \(A(\hat{e}_c)\)：某个样本经过映射器 \(A\) 后的特征
  可以理解为 anchor
  \item \(f_{text}(t_c)\)：正确类别 \(c\) 的文本特征
  可以理解为 positive
  \item \(f_{text}(t_{\cancel{c}})\)：错误类别的文本特征
  可以理解为 negative
  \item \(\text{dist}(\cdot,\cdot)\)：距离函数
  \item \(m\)：margin，间隔
\end{itemize}
...
\begin{itemize}
  \item anchor 离正确文本还不够近
  \item 或者离错误文本还不够远
  \item 或者二者之间没有拉开足够的 margin
\end{itemize}
...
\begin{itemize}
  \item \(a = A(\hat{e}_c)\)
  \item \(p = f_{text}(t_c)\)
  \item \(n = f_{text}(t_{\cancel{c}})\)
\end{itemize}

所以你完全可以把它理解成：
一个以“样本特征”为 anchor、以“正确/错误类别文本特征”为正负样本的 triplet-style loss。

\section{在训练中它具体会带来什么效果}

如果模型优化成功，那么特征空间会更像这样：

\begin{itemize}
  \item 属于类别 \(c\) 的样本，都会靠近文本原型 \(f_{text}(t_c)\)
  \item 并远离其他类别文本原型
  \item 类别之间边界更清晰
\end{itemize}
