...
\begin{itemize}
  \item \(x\) 表示当前状态（通常为带噪数据或 latent 表示）
  \item \(t\) 表示时间变量
  \item \(f_\theta\) 为由神经网络参数化的向量场
\end{itemize}
...
\section{一致性约束（Consistency Constraint）}
...
\begin{enumerate}
  \item 从 teacher 模型生成中间状态：
  
  \[
  x_t, x_{t'}
  \]
  \item 使用 teacher 模型计算目标输出：
