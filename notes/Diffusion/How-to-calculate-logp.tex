...
这种分析揭示了与LLMs的一个\colorbox{mdc2}{\textcolor{mdc1}{关键区别}}：与语言模型中离散令牌概率不同，扩散模型计算高斯状态转换概率。\colorbox{mdc2}{\textcolor{mdc1}{由此产生的二次项在对数重要性比中引入了依赖时间步的负偏置}}，如图2(b)所示。由于预期比率通常低于1，具有正优势的样本很少超过上剪辑边界。因此，来自过于自信的正预测的梯度大多被保留，而来自负样本的梯度则受到更严格的约束，从而使策略容易过度优化。
