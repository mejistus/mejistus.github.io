...
% ---
\href{https://www.cnblogs.com/qizhou/p/18564508}{泛函微分，是为变分。}

\section{Calculus of Variations}

Suggest \(J\) is the functional in \verb|Function Space| \(\mathcal{F}[a,b]\).
...
whereas \(y\) belongs to some permitted function which has continued \verb|1-order derivative|, with constraints \(y(a)=A, \quad y(b)=b\).
...
Then there exists a quantity \(\frac{\partial J}{\partial y(x)}\), called the \texttt{\(\textbf{functional derivative}\) of \(J\)}, such that
...
the first variation \texttt{\(\delta J[y;\delta y]\) is \(\textbf{the differential on the direction}\) \(\delta y\)}, can be computed explicitly as
...
So merge it to formula (4) we have \verb|derivative of functional|:
