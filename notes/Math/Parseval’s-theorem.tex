...
\verb|The total energy of a signal in the original domain is equal to the total energy of its representation in an orthonormal frequency basis.|

In signal processing, this means that \verb|a Fourier transform does not create or destroy energy|. It only redistributes the same energy among different frequency components.

For example, if a signal has large high-frequency Fourier coefficients, then a significant part of its total energy is carried by high-frequency oscillations. \verb|If a denoising or compression method removes those coefficients, the loss can be measured either in the spatial domain or in the frequency domain|. Parseval’s theorem guarantees that these two measurements are \verb|consistent|.

\section{Parseval’s theorem in finite-dimensional vector spaces}
...
\begin{enumerate}
  \item each basis vector has length \(1\);
  \item different basis vectors are perpendicular to each other.
\end{enumerate}
...
In other words, \verb|changing from the standard basis to another orthonormal basis may change the coordinates of the vector, but it does not change the total energy of the vector|.
...
\section{Discrete Fourier transform version}
...
\section{Fourier series version}
...
\section{Continuous Fourier transform version}
...
\section{Parseval’s theorem and Plancherel’s theorem}
...
\begin{itemize}
  \item Parseval’s theorem usually refers to the equality of energy between a function and its expansion coefficients under an orthonormal basis.
  \item Plancherel’s theorem states that the Fourier transform extends to a unitary operator on \(L^2\) spaces.
\end{itemize}
...
\begin{itemize}
  \item low frequencies describe global color, illumination, and coarse structure;
  \item middle frequencies describe edges, contours, local textures, and material patterns;
  \item high frequencies describe noise, sharp discontinuities, compression artifacts, and fine-grained pixel-level details.
\end{itemize}
...
\section{Application to reconstruction error}
...
\begin{enumerate}
  \item no normalization in the forward transform and \(1/N\) in the inverse transform;
  \item \(1/N\) in the forward transform and no normalization in the inverse transform;
  \item \(1/\sqrt{N}\) in both directions.
\end{enumerate}
