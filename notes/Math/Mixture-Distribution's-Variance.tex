...
这一步非常关键：
\texttt{在给定 \(G=g\) 时，\(X\) 的分布是由各个子分布 \(p(x\mid g,m)\) 按权重 \(p(m\mid g)\) 混合出来的。}
所以你要算 \(\mathrm{Var}(X\mid G=g)\)，当然不能只看 \(p(x\mid g,m)\)，还必须再对 \(m\) 的分布加权。

\section{为什么不是只对 \(p(x\mid g,m)\) 取期望}
...
\texttt{而给定 \(g\) 后，\(m\) 仍然是随机变量，所以总体上要对\(p(m\mid g)\)再做一次平均。}

所以这里一定会出现：

\begin{itemize}
  \item 内层对 \(p(x\mid g,m)\)
  \item 外层对 \(p(m\mid g)\)
\end{itemize}

\section{直接写成积分你就能看清楚}
...
\begin{itemize}
  \item 里面的 \(\mathbb{E}[X\mid G,M]\) 才是对 \(p(x\mid g,m)\) 积分
  \item 外面的 \(\mathbb{E}_{M\mid G}\) 是对 \(p(m\mid g)\) 积分
\end{itemize}
