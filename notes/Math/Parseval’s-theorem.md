...
Let $x \in \mathbb{C}^N$ be a vector. Suppose ${u_0,u_1,\dots,u_{N-1}}$ is an orthonormal basis of $\mathbb{C}^N$ under the inner product

$$
\langle x,y\rangle

\sum_{n=0}^{N-1} x_n \overline{y_n}.
$$

Since the basis is complete, $x$ can be expanded as
...
Because the basis vectors are orthonormal,
...
The energy of a vector is equal to the sum of the squared magnitudes of its coordinates under any orthonormal basis.

Thus, Parseval’s theorem is not only a theorem about Fourier analysis. It is a statement about orthonormal coordinate systems.
